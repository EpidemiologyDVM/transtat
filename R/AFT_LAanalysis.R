...

{
  ...
  LAdata <- read.csv('data/LAdata_2019-12.csv', header = TRUE)
...

{
  ...
  pairdata <- function(
    incpd=2, #Incubation period: Time from infection to clinical signs.
    latpd=0, #Latent period: Time from infection to infectious
    infpd=6  #Infectious period.
  ) {
    ...
    # infection time of index case # WJL Index for each household.
    LAdata$CTday <- sapply(
      LAdata$hhid, 
...
} #GENERATE PAIRWISE DATA FOR SURVIVAL OBJ
...
#Neither tested interaction is signficant
...
} #TEST EFFECTS OF PROPHYLAXIS PREDICTORS
